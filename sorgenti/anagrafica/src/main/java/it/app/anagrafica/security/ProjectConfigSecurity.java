package it.app.anagrafica.security;

import it.app.anagrafica.component.JWTAuthenticationProvider;
import it.app.anagrafica.filter.JwtAuthenticationFilter;
...
public class ProjectConfigSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
        http.addFilterBefore(authenticationFilter(), BasicAuthenticationFilter.class).authorizeRequests().
				antMatchers("/user/create").hasAnyAuthority("Client","Admin").
				antMatchers("/user/update").hasAnyAuthority("Client","Admin").
				antMatchers("/user/delete").hasAnyAuthority("Client","Admin").
